...
class _BaseSupLossAReisz(_BaseAReisz):

    def fit(self, Z, T, Y, moment_func,
        ...
        for epoch in range(n_epochs):
            ...
            for it, (zb, xb, yb) in enumerate(self.train_dl):
                ...
                if (it % train_learner_every == 0):
                    self.learner.train()
                    pred = self.learner(zb)
                    test = self.adversary(xb)
                    m = moment_func(self.adversary, xb)
                    D_loss = torch.mean(
                        (m - pred*test - test**2))
                ...
                if (it % train_adversary_every == 0):
                    ...
                    if self.adversary_reg:
                        test, reg = self.adversary(xb, reg=True)
                    else:
                        test = self.adversary(xb)
                    m = moment_func(self.adversary, xb)    
                    G_loss = - torch.mean((m - pred*test - test**2))
...
class AReisz(_BaseSupLossAReisz):
